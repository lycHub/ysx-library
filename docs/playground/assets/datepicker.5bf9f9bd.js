...
}`,q=`import { Picker, } from "@ysx-libs/mobile-picker";\r
...
function getListHtmlStr(list: string[]) {\r
  ...
  return str;\r
}`;const v={value:null,mode:C};let o={};const c=D(v.mode);let l=[],d=[];function F(){const r=A(v);o=r.allData,l=r.formattedValue;const e=c.map(i=>o[i].data);d=u(),B(e),f();const t=new H(".mobile-picker",{selectedIndexes:d,onChange(i,a,n){["wheel","drag","click"].includes(a)&&$.call(t,i,n)}})}function $(r,e){var a;const t=c[e],i=l.find(n=>n.type===t);if(!!i){if(i.value=o[t].data[r[e]],i.pickerIndex=r[e],t==="M"){const{newValue:n,newIndex:p,newOptions:g,valueChanged:y}=S(l,(a=v.filter)==null?void 0:a.d);o.d.data=g;const x=c.map(s=>o[s].data),k=c.indexOf("d");if(N(x,k),y){const s=l.find(w=>w.type==="d");s.value=n.toString(),s.pickerIndex=p}d=u(),this.refreshColumns([k],d)}d=u(),f()}}function u(){return c.map(r=>{const e=l.find(t=>t.type===r);return(e==null?void 0:e.pickerIndex)||0})}function f(){const r=M(l),e=document.querySelector(".demo-card-top .value span");e&&(e.textContent=P(r,"yyyy/MM/dd HH:mm"))}function A(r){const e=T(r.value,r.filter);return{allData:e,formattedValue:Object.entries(e).map(([t,{selected:i,data:a}],n)=>({index:n,type:t,value:i,pickerIndex:Math.max(a.findIndex(p=>p===i),0)}))}}function B(r){const e=document.querySelector(".mobile-picker .mobile-picker-view-container");if(e){let t="";r.forEach(i=>{t+=`<div class="mobile-picker-view">
          <div class="mobile-picker-view-item-container">
            ${b(i)}
          </div>
        </div>`}),e.innerHTML=t}}function N(r,e){const t=document.querySelector(`.mobile-picker .mobile-picker-view-container .mobile-picker-view:nth-child(${e+1}) .mobile-picker-view-item-container`);if(t){let i="";i+=b(r[e]),t.innerHTML=i}}function b(r){let e="";return r.forEach(t=>{e+=`<div class="mobile-picker-view-item">${t}</div>`}),e}m.registerLanguage("typescript",h);m.registerLanguage("html",I);m.registerLanguage("scss",V);const G={html:E,css:j,js:q,types:O,utils:L};function R(){document.querySelectorAll(".demo-code-content").forEach(e=>{e.querySelector("pre code").textContent=G[e.value]})}customElements.whenDefined("mdui-card").then(()=>{F(),R(),m.highlightAll()});
